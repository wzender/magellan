/**
 * Migration: add attributes_en JSONB column to all existing per-run tables
...
async function migrate() {
  ...
  try {
    await query(`ALTER TABLE run_results ADD COLUMN IF NOT EXISTS attributes_en JSONB`);
    console.log('✓ run_results.attributes_en added (or already existed)');
  } catch (err) {
    if (err.code === '42P01') {
  ...
  for (const { run_id: tbl } of runRows) {
    try {
      await query(`ALTER TABLE "${tbl}" ADD COLUMN IF NOT EXISTS attributes_en JSONB`);
      console.log(`  ✓ "${tbl}".attributes_en added`);
    } catch (err) {
      if (err.code === '42P01') {
