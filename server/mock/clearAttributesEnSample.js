/**
 * Clears attributes_en for 100 random records per benchmark so the
...
Object.entries(byBenchmark).forEach(([benchmarkId, runFiles]) => {
  ...
  // Clear attributes_en in every run file for this benchmark
  runFiles.forEach(fname => {
    ...
    const updated = runRows.map(r => sampled.has(r.request_id) ? { ...r, attributes_en: '' } : r);
...
console.log(`\nDone. Cleared attributes_en for ${totalCleared} unique request_ids (${SAMPLE_SIZE} per benchmark).`);
