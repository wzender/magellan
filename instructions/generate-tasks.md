...
Identify Relevant Files: Based on the tasks and requirements, identify potential files that will need to be created or modified. List these under the Relevant Files section, including corresponding test files if applicable.

Generate Final Output: Combine the parent tasks, sub-tasks, relevant files, and notes into the final Markdown structure.

Save Task List: Save the generated document in the /tasks/ directory with the filename tasks-[feature-name].md, where [feature-name] describes the main feature or task being implemented (e.g., if the request was about user profile editing, the output is tasks-user-profile-editing.md).
